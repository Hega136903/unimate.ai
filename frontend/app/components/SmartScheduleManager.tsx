...
const SmartScheduleManager: React.FC = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      // derive start/end for selected view
      ...
      const url = `${API_BASE_URL}/schedule/analytics?startDate=${start.toISOString()}&endDate=${end.toISOString()}`;
      const response = await fetch(url, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  // Track items we've auto-updated to 'missed' to avoid spamming updates
  const [autoMissedSet, setAutoMissedSet] = useState<Set<string>>(new Set());

  // Helper: update item status (completed/missed)
  const updateItemStatus = async (itemId: string, status: 'completed' | 'missed') => {
    const token = getAuthToken();
    if (!token) return;
    try {
      ...
      if (!response.ok) {
        ...
        console.error('📅 Status update failed:', response.status, err);
        showNotification('Failed to update status', 'error');
        return;
      }
      // Refresh schedule and analytics
      await fetchSchedule();
      await fetchAnalytics();
      showNotification(status === 'completed' ? 'Marked as completed' : 'Marked as missed', 'success');
    } catch (e) {
  ...
  // Auto-mark items as missed if their endTime passed and not completed/cancelled/missed
  useEffect(() => {
    if (schedule.length === 0) return;
    const now = Date.now();
    const toMark = schedule.filter(item => {
      const ended = new Date(item.endTime).getTime() < now;
      const eligible = item.status === 'scheduled' || item.status === 'in-progress';
      const notAlready = !autoMissedSet.has(item.id);
      return ended && eligible && notAlready;
    });
    if (toMark.length === 0) return;
    // Update sequentially to avoid request bursts
    (async () => {
      const newSet = new Set(autoMissedSet);
      for (const item of toMark) {
        await updateItemStatus(item.id, 'missed');
        newSet.add(item.id);
      }
      setAutoMissedSet(newSet);
    })();
  }, [schedule]);
  ...
  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          {/* Actions: mark as completed/missed */}
                          {(item.status !== 'completed' && item.status !== 'missed' && item.status !== 'cancelled') && (
                            <div className="mt-3 flex gap-2">
                              ...
                              <button
                                onClick={() => updateItemStatus(item.id, 'missed')}
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
      <p className="text-sm text-gray-600">{selectedView === 'day' ? 'Today' : selectedView === 'week' ? 'This Week' : 'This Month'}</p>
                    ...
                    <div className="space-y-3">
                      {suggestions.studyRecommendations.map((rec, index) => (
                        <div key={index} className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                          ...
                          {/* Create Study Session button removed as requested */}
                        </div>
                      ))}
                    </div>
                  </div>
